          ...
          <div className="space-y-3">
            {surgeryOptions.map((option) => (
              <button
                ...
                className={`w-full p-4 text-left rounded-2xl transition-all flex items-center border-2 hover:border-[#4fa87f] ${
                  selectedItems.includes(option.id)
                    ? 'bg-[#f0feab] border-[#4fa87f]'
                    : 'bg-white border-gray-200'
                }`}
              >
                <div className={`w-5 h-5 flex-shrink-0 rounded border-2 flex items-center justify-center mr-3 transition-all ${
                  selectedItems.includes(option.id) ? 'bg-[#413d3d] border-[#413d3d]' : 'bg-white border-gray-300'
                }`}>
                  {selectedItems.includes(option.id) && (
