  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="px-6 lg:px-8 pt-6">
        <Link href="/intake/consent" className="inline-block p-2 -ml-2 hover:bg-white/10 rounded-lg">
          ...
          <div className="mt-6">
            <label className="flex items-start cursor-pointer" onClick={() => setTermsAccepted(!termsAccepted)}>
              <div className={`mt-0.5 w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 ${
                termsAccepted ? 'bg-white border-[#413d3d]' : 'border-white/60 bg-transparent'
              }`}>
                {termsAccepted && (
              ...
              <span className="ml-3 text-[10.5px] md:text-[12.5px] text-white/80 leading-tight">
                {selectedState === 'Florida' ? (
                  ...
                  <>
                    {t('state.terms.agree')}<a href={isSpanish ? "https://www.eonmeds.com/terminosycondiciones" : "https://www.eonmeds.com/termsandconditions"} target="_blank" rel="noopener noreferrer" className="text-[#f0feab] underline" onClick={(e) => e.stopPropagation()}>{t('state.terms.link')}</a>, <a href="#" className="text-[#f0feab] underline" onClick={(e) => e.stopPropagation()}>{t('state.terms.njConsent')}</a>, <a href={isSpanish ? "https://www.eonmeds.com/consentimientodetelemedicina" : "https://www.eonmeds.com/telehealth-consent"} target="_blank" rel="noopener noreferrer" className="text-[#f0feab] underline" onClick={(e) => e.stopPropagation()}>{t('state.terms.telehealth')}</a>{t('state.terms.acknowledge')}<a href={isSpanish ? "https://www.eonmeds.com/politicadeprivacidad" : "https://www.eonmeds.com/privacypolicy"} target="_blank" rel="noopener noreferrer" className="text-[#f0feab] underline" onClick={(e) => e.stopPropagation()}>{t('state.terms.privacy')}</a>.
                  </>
                ) : (
                  <>
                    {t('state.terms.generic')}<a href={isSpanish ? "https://www.eonmeds.com/terminosycondiciones" : "https://www.eonmeds.com/termsandconditions"} target="_blank" rel="noopener noreferrer" className="text-[#f0feab] underline" onClick={(e) => e.stopPropagation()}>{t('state.terms.link')}</a> {t('state.terms.and')} <a href={isSpanish ? "https://www.eonmeds.com/consentimientodetelemedicina" : "https://www.eonmeds.com/telehealth-consent"} target="_blank" rel="noopener noreferrer" className="text-[#f0feab] underline" onClick={(e) => e.stopPropagation()}>{t('state.terms.telehealth')}</a> {t('state.terms.acknowledge')}<a href={isSpanish ? "https://www.eonmeds.com/politicadeprivacidad" : "https://www.eonmeds.com/privacypolicy"} target="_blank" rel="noopener noreferrer" className="text-[#f0feab] underline" onClick={(e) => e.stopPropagation()}>{t('state.terms.privacy')}</a>.
                  </>
                )}
              </span>
            </label>
